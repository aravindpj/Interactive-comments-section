...
function CommentDisplay({comment,opnDelModel,setDelModel,setReply,reply,editComment}) {
    ...
    <div className='w-full '>
        ...
        <div className=' flex justify-between items-center'>
            ...
            <div className='hidden sm:flex gap-2 items-center cursor-pointer '>
                {
                    !comment?.currentUser && 
                    <button onClick={()=>setReply(!reply)} className='flex gap-2 items-center'>
...

                {
                    comment?.currentUser && 
                    <>
                       <button onClick={()=>setDelModel(!opnDelModel)} className='flex items-center gap-1.5 mr-5' >
        ...
        {/*comment*/}
        {
            !edit ? 
            <div className='  mt-2'>
                <p className='text-16px break-words pr-6 text-gray-500 leading-normal'>
                  {comment?.content}
                </p>
            </div> :
            <form onSubmit={handleEditForm} className='flex flex-col items-end mt-4 gap-5'>
                <textarea onChange={(e)=>setUpdateContent(e.target.value)}  name="" placeholder='Update reply...' id=""  className=" w-full text-base font-thin px-3 py-2 flex-1 text-gray-700 outline-none resize-none border border-solid border-gray-400 border-b rounded-lg"></textarea>
                <button className='uppercase px-7 h-12 bg-modblue-100 font-semibold text-white rounded-lg'>Update</button>
            </form>
        }
    </div>
  )
